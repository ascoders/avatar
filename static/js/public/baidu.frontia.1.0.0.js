...
baidu.frontia = {},
	function (e) {
		baidu.frontia.version = "1.0.0", baidu.frontia.serviceHost = {
			...
			t.toJSON(), baidu.frontia.localStorage.setItem(baidu.frontia._getLocalStorageKey(baidu.frontia._CURRENT_USER_KEY), JSON.stringify(t)), baidu.frontia.currentAccount = t
	...
	function () {
		...
		function s(e, t) {
			function r(e, t) {
				...
				var s, c;
				for (s = 0, c = a.length; c > s; ++s) y[_++] = a[s];
				v[a[0]] ++, b[a[3]] ++, A = e.length + t - 1, l = null
			}
			var n, a, o, i, s, u, f, l, p, g = {},
				y = m ? new Uint16Array(2 * t.length) : [],
				_ = 0,
				A = 0,
				v = new(m ? Uint32Array : Array)(286),
			...
			for (v[256] = 1, n = 0, a = t.length; a > n; ++n) {
				...
				if (g[s] === h && (g[s] = []), u = g[s], !(0 < A--)) {
					...
					if (n + 3 >= a) {
						for (l && r(l, -1), o = 0, i = a - n; i > o; ++o) p = t[n + o], y[_++] = p, ++v[p];
						break
					}
		...
		function c(e, t, r) {
			...
			e: for (; u > s; s++) {
				...
				for (; 258 > o && l > t + o && e[n + o] === e[t + o];) ++o;
				if (o > f && (a = n, f = o), 258 === o) break
			}
			return new i(f, t - a)
		}
		...
		function u(e, t) {
			var r, n, o, i, s, c = e.length,
				u = new a(572),
				l = new(m ? Uint8Array : Array)(c);
			if (!m)
				for (i = 0; c > i; i++) l[i] = 0;
			for (i = 0; c > i; ++i) 0 < e[i] && u.push(i, e[i]);
			if (r = Array(u.length / 2), n = new(m ? Uint32Array : Array)(u.length / 2), 1 === r.length) return l[u.pop().index] = 1, l;
			for (i = 0, s = u.length / 2; s > i; ++i) r[i] = u.pop(), n[i] = r[i].value;
			for (o = f(n, n.length, t), i = 0, s = r.length; s > i; ++i) l[r[i].index] = o[i];
			return l
		}
		...
		function f(e, t, r) {
			...
			var a, o, i, s, c, u = new(m ? Uint16Array : Array)(r),
				f = new(m ? Uint8Array : Array)(r),
				l = new(m ? Uint8Array : Array)(t),
				p = Array(r),
				h = Array(r),
				d = Array(r),
				g = (1 << r) - t,
				y = 1 << r - 1;
			for (u[r - 1] = t, o = 0; r > o; ++o) y > g ? f[o] = 0 : (f[o] = 1, g -= y), g <<= 1, u[r - 2 - o] = (0 | u[r - 1 - o] / 2) + t;
			for (u[0] = f[0], p[0] = Array(u[0]), h[0] = Array(u[0]), o = 1; r > o; ++o) u[o] > 2 * u[o - 1] + f[o] && (u[o] = 2 * u[o - 1] + f[o]), p[o] = Array(u[o]), h[o] = Array(u[o]);
			for (a = 0; t > a; ++a) l[a] = r;
		...
		for (y = 0; 256 > y; ++y) {
			for (var A = y, v = A, b = 7, A = A >>> 1; A; A >>>= 1) v <<= 1, v |= 1 & A, --b;
			_[y] = (255 & v << b) >>> 0
		}
		var w = _,
		...
		o.prototype.g = function () {
			...
			case 0:
				for (n = 0, a = o.length; a > n;) {
					...
					var i = r,
						...
						v = this.c;
					if (m) {
						for (A = new Uint8Array(this.a.buffer); A.length <= v + i.length + 5;) A = new Uint8Array(A.length << 1);
						A.set(this.a)
					}
					if (f = c ? 1 : 0, A[v++] = 0 | f, p = i.length, g = 65535 & ~p + 65536, A[v++] = 255 & p, A[v++] = 255 & p >>> 8, A[v++] = 255 & g, A[v++] = 255 & g >>> 8, m) A.set(i, v), v += i.length, A = A.subarray(0, v);
					else {
						for (y = 0, _ = i.length; _ > y; ++y) A[v++] = i[y];
						A.length = v
					}
					this.c = v, this.a = A
				}
			...
			case 1:
				...
				for (w = 0, T = k.length; T > w; w++)
					if (S = k[w], t.prototype.b.apply(b, I[S]), S > 256) b.b(k[++w], k[++w], d), b.b(k[++w], 5), b.b(k[++w], k[++w], d);
					else if (256 === S) break;
				this.a = b.finish(), this.c = this.a.length;
				break;
			case j:
				var x, O, R, D, N, P, E, C, U, L, M, q, F, K, J, B = new t(m ? new Uint8Array(this.a.buffer) : this.a, this.c),
				...
				for (H = G = 0; Z > H; H++) et[G++] = P[H];
				for (H = 0; W > H; H++) et[G++] = C[H];
				if (!m)
					for (H = 0, Y = rt.length; Y > H; ++H) rt[H] = 0;
				for (H = $ = 0, Y = et.length; Y > H; H += G) {
					...
					if (Q = G, 0 === et[H])
						if (3 > Q)
							...
							for (; Q > 0;) X = 138 > Q ? Q : 138, X > Q - 3 && Q > X && (X = Q - 3), 10 >= X ? (tt[$++] = 17, tt[$++] = X - 3, rt[17] ++) : (tt[$++] = 18, tt[$++] = X - 11, rt[18] ++), Q -= X;
					else if (tt[$++] = et[H], rt[et[H]] ++, Q--, 3 > Q)
						for (; 0 < Q--;) tt[$++] = et[H], rt[et[H]] ++;
					else
						for (; Q > 0;) X = 6 > Q ? Q : 6, X > Q - 3 && Q > X && (X = Q - 3), tt[$++] = 16, tt[$++] = X - 3, rt[16] ++, Q -= X
				}
				for (e = m ? tt.subarray(0, $) : tt.slice(0, $), L = u(rt, 7), K = 0; 19 > K; K++) V[K] = L[z[K]];
				for (N = 19; N > 4 && 0 === V[N - 1]; N--);
				for (M = l(L), B.b(R - 257, 5, d), B.b(D - 1, 5, d), B.b(N - 4, 4, d), K = 0; N > K; K++) B.b(V[K], 3, d);
				...
				var nt, at, ot, it, st, ct, ut, ft, lt = [E, P],
					pt = [U, C];
				for (st = lt[0], ct = lt[1], ut = pt[0], ft = pt[1], nt = 0, at = O.length; at > nt; ++nt)
					if (ot = O[nt], B.b(st[ot], ct[ot], d), ot > 256) B.b(O[++nt], O[++nt], d), it = O[++nt], B.b(ut[it], ft[it], d), B.b(O[++nt], O[++nt], d);
		...
		p.prototype.g = function () {
			var e, t, r, a, i, s, c, u, f = new(m ? Uint8Array : Array)(32768),
				l = 0,
				p = this.input,
				d = this.i,
				g = this.filename,
			...
			if (f[l++] = 31, f[l++] = 139, f[l++] = 8, e = 0, this.d.fname && (e |= N), this.d.fcomment && (e |= P), this.d.fhcrc && (e |= D), f[l++] = e, t = 0 | (Date.now ? Date.now() : +new Date) / 1e3, f[l++] = 255 & t, f[l++] = 255 & t >>> 8, f[l++] = 255 & t >>> 16, f[l++] = 255 & t >>> 24, f[l++] = 0, f[l++] = R, this.d.fname !== h) {
				for (c = 0, u = g.length; u > c; ++c) s = g.charCodeAt(c), s > 255 && (f[l++] = 255 & s >>> 8), f[l++] = 255 & s;
				f[l++] = 0
			}
			if (this.d.comment) {
				for (c = 0, u = y.length; u > c; ++c) s = y.charCodeAt(c), s > 255 && (f[l++] = 255 & s >>> 8), f[l++] = 255 & s;
				f[l++] = 0
			}
	...
	function (e) {
		...
		n.hooks = {}, n.hooks.serializeData = function (e) {
			...
			if (i.open(s, u, !0), e.header) {
				var f;
				for (f in e.header) e.header.hasOwnProperty(f) && i.setRequestHeader(f, e.header[f])
			}
			if ("GET" === s) i.send();
			else {
	...
	function (e) {
		e.jsonp = function () {
			...
			function r(r, n, a, c, u) {
				...
				n = n || {};
				for (f in n) n.hasOwnProperty(f) && (l += t(f) + "=" + t(n[f]) + "&");
	...
	function (e) {
		...
		function r(t) {
			...
			var n = {},
				a = new Zlib.Gzip(new Uint8Array(JSON.stringify(r).split("").map(function (e) {
		...
		e.util.mix(e.User, u), e.Role = e.Account.extend({
			...
			constructor: function (t, r) {
				this.roleList = {}, Array.isArray(r) && r.forEach(function (t) {
		...
		e.util.mix(e.Role, f), e.ACL = e.Base.extend({
			permissions: null,
	...
	}(baidu.frontia), baidu.frontia.social = baidu.frontia.social || {},
	function (e) {
		function t(e) {
			return encodeURIComponent(e + "")
		}
		...
		function n(t) {
			var n = e.getApiKey(),
				a = r(n, t.response_type, t.media_type, t.redirect_uri, t.state, t.display, t.client_type, t.scope);
			return a
		}
		...
		function s(t) {
			...
			n.stats = btoa(String.fromCharCode.apply(null, o));
			var i = e.ajax;
			i.post(p, JSON.stringify(n), "json", {
				contentType: "application/json"
			})
		}
		...
		var u = e.error,
			f = e.DomainManager.getSocialDomain() + "/social/oauth/2.0/authorize",
			l = e.DomainManager.getSocialDomain() + "/social/api/2.0/user/info",
			p = e.DomainManager.getPBLogDomain() + "/pushlog",
			h = e.DomainManager.getFrontiaDomain() + "/user",
			d = {
				TOKEN: "token",
				CODE: "code"
			},
			g = {
				...
				login: function (t) {
					if (e.logOutCurrentAccount(), !t) throw new baidu.frontia.error(baidu.frontia.ERR_MSG.INVALID_PARAMS);
					if (t.error = t.error || function () {}, !t.response_type) throw new baidu.frontia.error(baidu.frontia.ERR_MSG.INVALID_PARAMS);
		...
		var u = {
			...
			uploadFile: function (o, i) {
				...
				if (i = i || {}, u._configure(i), u._checkParams([{
					...
					var f = {
						...
						p = e.ajax;
					p.post(s, JSON.stringify(l), "json", {
						header: {
							authorization: t(e.getApiKey())
						},
						contentType: "application/json",
			...
			findData: function (o, s) {
				...
				if (s = s || {}, this._configure(s), this._checkParams([{
					...
					var f = this._attachAccount({
							method: "query",
							criteria: o.query
						}),
						l = this._createAjaxOpt(c, {
							header: {
								authorization: t(e.getApiKey())
							},
							contentType: "application/json"
		...
		u = {
			...
			getFileUrl: function (i, s) {
				...
				if (s = s || {}, u._configure(s), u._checkParams([{
					...
					var l = t(i, s, ["success", "error"]),
						p = r("file?", l.query);
					s.method = "meta", l = t(i, s, ["success", "error"]);
					var h = r("file?", l.query),
						d = {
							callback: function (e) {
								if (e.error_code) {
									var t = new o(e);
	...
	function (e) {
		...
		var o = {
			...
			send: function (n, o) {
				...
				for (var f in o) o.hasOwnProperty(f) && "success" !== f && "error" !== f && (u[f] = o[f]);
				var l = s._createAjaxOpt({
						header: {
							authorization: t(e.getApiKey())
						},
						contentType: "application/json"
